...
export class RainViewerService {
    ...
    public static readonly COLOR_SCHEMES = [
        ...
        { id: 2, name: "Universal Blue", colors: ["#d6e2ff", "#b5c9ff", "#8eafff", "#7d96ff", "#6b7dff", "#5662ff", "#2932ff", "#1218bd", "#f7ff00", "#ffc800", "#ff9600", "#ff6400", "#ff0000", "#c80000", "#960000", "#640000"] }, // Similar to 1 but smoother? Actually API doc says 2 is "Universal Blue"
    ...
    private static hexToRgb(hex: string): { r: number, g: number, b: number } | null {
        ...
        const scheme = this.COLOR_SCHEMES.find(s => s.id === this.colorScheme) || this.COLOR_SCHEMES[0];
        
        let minDist = Infinity;
        let bestIndex = -1;
        
        scheme.colors.forEach((hex, index) => {
            if (hex.startsWith('#00000000')) return; // Skip transparent
        ...
        // CHANGED: Remove the minDist > 60 check, or increase threshold significantly
        if (bestIndex === -1) return null;
        
        // Map to dBZ: 0→10, 1→15, 2→20, ..., 15→85
